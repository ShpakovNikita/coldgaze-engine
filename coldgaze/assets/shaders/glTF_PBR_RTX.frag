...
// Result reflection
vec3 BRDF_CookTorrance(float roughness, float metallic, vec3 albedo, vec3 N, vec3 V, vec3 F0, vec3 lightPos, vec3 lightColor)
{
    vec3 L = normalize(inWorldPos - lightPos);	
...
void main() 
{
    ...
    if (material.workflow == PBR_WORKFLOW_METALLIC_ROUGHNESS)
    {
        ...
        vec3 V = normalize(uboScene.cameraPos.xyz - inWorldPos);
