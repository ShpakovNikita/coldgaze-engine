...
void CG::Vk::CubeTexture::LoadFromFile(const std::string& fileName, Device* device, VkQueue copyQueue)
{
    ...
    if (data)
    {
        ...
        FromBuffer(data, imageSize, VK_FORMAT_R32G32B32_SFLOAT, width, height, device, copyQueue);

        stbi_image_free(data);
    }
    else
    {
        throw AssetLoadingException("Failed to load hdr environment map! Make sure that it has .hdr extension and RGBE format!");
    }
}

void CG::Vk::CubeTexture::FromBuffer(
    const void* buffer, 
